...
func OpenaiTTSHandler(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (*dto.Usage, *types.NewAPIError) {
	// Do not commit the response until actual audio data is available. Once audio
	// has been written, later stream/body failures remain non-retryable.
	...
	if info.IsStream {
		...
		helper.StreamScannerHandler(c, resp, info, func(data string, sr *helper.StreamResult) {
			...
			hasAudio := strings.TrimSpace(streamResponse.Audio) != ""
		...
		if !hasAudioData {
			...
			return nil, helper.NewEmptyResponseError("upstream returned no audio data")
		}
		return usage, nil
	}
