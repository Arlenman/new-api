...
interface UpstreamChannelDistributionChartProps {
  data: UpstreamChannelRankingData
  metric: UpstreamChannelStatisticsMetric
}

export function UpstreamChannelDistributionChart({
  data,
  metric,
}: UpstreamChannelDistributionChartProps) {
  ...
  const spec = useMemo(() => {
    if (data.data.length === 0 || data.total <= 0) return null
    return {
      type: 'pie' as const,
      data: [{ id: 'upstream-channel-distribution', values: data.data }],
      categoryField: 'label',
      valueField: 'value',
      outerRadius: 0.72,
      innerRadius: 0,
      ...
      label: {
        ...
        style: { fill: chartTextColor, fontSize: 12, fontWeight: 500 },
        line: { visible: true, style: { stroke: chartLineColor } },
      },
      tooltip: {
        mark: {
          title: {
  ...
  return (
    <div className='h-[360px]'>
      <VChart
        key={`upstream-channel-distribution-${metric}-${resolvedTheme}`}
        spec={{
          ...spec,
