...
import {
  ...
  type UpstreamChannelTrendData,
} from '../statistics-lib'

interface UpstreamChannelTrendChartProps {
  data: UpstreamChannelTrendData
  metric: UpstreamChannelStatisticsMetric
}

export function UpstreamChannelTrendChart({
  data,
  metric,
}: UpstreamChannelTrendChartProps) {
  ...
  const visibleData = useMemo(
    () => filterUpstreamChannelTrendData(data, hiddenSeriesKeys),
  ...
  const spec = useMemo(() => {
    ...
    return {
      type: 'line' as const,
      data: [{ id: 'upstream-channel-trend', values: visibleData.data }],
      xField: 'periodLabel',
      yField: 'value',
      seriesField: 'seriesKey',
      line: { style: { lineWidth: 2, stroke: getSeriesColor } },
      point: {
        visible: true,
        style: {
          size: 5,
    ...
    <div className='space-y-3'>
      <div className='overflow-x-auto pb-2'>
        <div className='h-[360px]' style={{ width: chartWidth }}>
          {spec ? (
            <VChart
              key={`upstream-channel-trend-${metric}-${resolvedTheme}-${[...hiddenSeriesKeys].sort().join('-')}`}
              spec={{
                ...spec,
            ...
            <div className='text-muted-foreground flex h-full items-center justify-center text-sm'>
              {t('All channels are hidden. Click a channel below to show it.')}
            </div>
          )}
        </div>
      </div>
