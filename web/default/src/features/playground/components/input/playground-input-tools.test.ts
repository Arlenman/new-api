...
const playgroundInputSource = readFileSync(
  new URL('./playground-input.tsx', import.meta.url),
  'utf8'
)
...
describe('PlaygroundInputTools attachment entry', () => {
  ...
  test('allows selecting multiple attachments in one file picker interaction', () => {
    assert.match(playgroundInputSource, /<PromptInput[\s\S]*\bmultiple\b/)
  })
})
